...
class HookGenerator:
    ...
    def _register(self, sb) -> str:
        ...
        # Method 1 (primary): Type via Selenium send_keys — fires real React events
        try:
            pw_elem = sb.find_element("input[type='password']")
            pw_elem.click()
            self._wait(0.2)
            # Type character by character for Clerk's React validation
            for ch in password:
                pw_elem.send_keys(ch)
                self._wait(0.03)
            self._wait(0.2)
            pw_filled = True
            print("  ✓ Filled password via send_keys")
        except Exception as e:
            print(f"  ⚠ Password send_keys failed: {e}")

        # Method 2 (fallback): nativeInputValueSetter for React if send_keys failed
        if not pw_filled:
            try:
                ...
                if result == 'ok':
                    pw_filled = True
                    print("  ✓ Filled password (React setter fallback)")
            except Exception:
                pass
        ...
        # Wait briefly for Clerk's React state to register the password
        self._wait(0.5)
        submitted = False

        # Method 1: Click the "Continue with Email" button (primary submit for Clerk)
        # The Clerk form uses this button as the main submit trigger, NOT form.submit()
        try:
            result = self._js(sb, """return (function(){
                var btns = document.querySelectorAll('button');
                for (var b of btns) {
                    var txt = b.textContent.trim();
                    if ((txt === 'Continue with Email' || txt === 'Continue with email')
                        && b.offsetWidth > 0) {
                        b.click();
                ...
                if (submitBtn && submitBtn.offsetWidth > 0) {
                    submitBtn.click();
                    return 'clicked-submit-btn';
                }
                return null;
            })()""")
            if result:
                submitted = True
                print(f"  ✓ Submitted form: {result}")
        except Exception as e:
            print(f"  ⚠ Button click error: {e}")

        # Method 2: Enter key on password input
        if not submitted:
            try:
                sb.send_keys("input[type='password']", "\n")
                submitted = True
        ...
        # Method 3: ActionChains click on submit button
        if not submitted:
            try:
                ...
                if btn:
                    ...
                    submitted = True
                    print("  ✓ Clicked Submit button (ActionChains)")
            except Exception as e:
                print(f"  ⚠ Submit button click error: {e}")

        # Method 4: Enter on email input
        if not submitted:
            try:
                sb.send_keys("input[type='email']", "\n")
                submitted = True
                print("  ✓ Submitted via Enter key on email")
            except Exception:
                pass
        if not submitted:
        ...
        # Immediate error check: catch password/form validation errors early
        try:
            form_error = self._js(sb, """return (function(){
                var errs = document.querySelectorAll(
                    '.cl-formFieldErrorText, [data-localization-key*="error"],' +
                    '[class*="formFieldError"], [class*="field__error"],' +
                    '.cl-alert__text, [role="alert"]'
                );
                var texts = [];
            ...
            if form_error:
                print(f"  ⚠ Clerk form error after submit: {form_error}")
                if any(w in form_error.lower() for w in
                       ["password", "too short", "too weak", "at least",
                        "characters", "uppercase", "number"]):
                    print(f"  ℹ Password issue detected, re-typing password with send_keys...")
                    try:
                        pw_elem = sb.find_element("input[type='password']")
                        pw_elem.click()
                        pw_elem.clear()
                        ...
                        self._js(sb, """return (function(){
                            ...
                            for (var b of btns) {
                                ...
                                if ((txt === 'Continue with Email' || txt === 'Continue with email')
                                    && b.offsetWidth > 0) {
                                    b.click(); return 'resubmitted';
                                }
                            }
                        })()""")
                        self._wait(1.5)
                        print("  ✓ Re-submitted after password fix")
                    except Exception as e:
                        print(f"  ⚠ Password re-entry failed: {e}")
        except Exception:
            pass
        ...
        verify_screen_found = False
        for _ in range(10):
            try:
                body_text = self._js(sb,
                    "return (document.body.innerText||'').substring(0,800)") or ""
                # Verification screen found
                if ("Verify Your Email" in body_text
                    ...
                    verify_screen_found = True
                    break
                # Already authenticated — skip OTP entirely
                if ("How do you plan" in body_text
                        or "Motion Control" in body_text
                        or "Create Video" in body_text
                        or "Generate" in body_text):
                    print("  ✅ Already authenticated (skipped OTP)!")
                    return email
            except Exception:
                pass
            self._wait(0.5)

        if not verify_screen_found:
            # Check: are we still on signup/login screen?
            try:
                body_now = self._js(sb,
                    "return (document.body.innerText||'').substring(0,500)") or ""
                if ("Continue with Email" in body_now
                        or "Continue with Google" in body_now
                        or "Welcome to Higgsfield" in body_now
                        or "Sign up" in body_now):
                    # Form likely wasn't submitted — try re-submit once
                    print("  ⚠ Still on signup screen — trying to re-submit form...")
                    try:
                        sb.send_keys("input[type='password']", "\n")
                    except Exception:
                        try:
                    ...
                    self._wait(2)
                    # Check again
                    body_retry = self._js(sb,
                        "return (document.body.innerText||'').substring(0,500)") or ""
                    if ("Verify" in body_retry or "verification" in body_retry
                            or "Enter the code" in body_retry):
                        print("  ✓ Verification screen appeared after re-submit")
                        verify_screen_found = True
                    elif ("Continue with Email" in body_retry
                            or "Continue with Google" in body_retry):
                        raise RuntimeError(
                            "Form submission failed — still on signup screen after re-submit")
                    else:
                        print("  ⚠ Unknown page state after re-submit, trying OTP anyway")
                else:
                    print("  ⚠ Verification screen not detected, trying OTP anyway")
            except RuntimeError:
                raise
            except Exception:
                print("  ⚠ Could not check page state, trying OTP anyway")
                    ...
                    try:
                        ...
                        try:
                            ...
                            if ("Continue with Email" in body_check
                                    ...
                                    or "Welcome to Higgsfield" in body_check
                                    or "Sign up" in body_check
